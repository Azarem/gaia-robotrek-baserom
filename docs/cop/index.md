...
- COP opcodes defined in copdef.json: **251**
...
- **Unused Ops** (`7F`): The sole invalid slot in the 251-entry COP jump table. Entry is `#$FFFF`, which would jump to `$00:FFFF` (interrupt vector area) and crash. 0 call sites. Every other zero-use opcode still has a valid handler — `[7F]` is uniquely dead. Likely an intentional reservation at the `$7E`/`$80` boundary.
...
- **Velocity Set** (`B4`–`B6`): Three opcodes that set actor velocity fields (`$1C` / `$1E`) via lookup in velocity table `unk29_list_01C3B9`. `[B4]` (10 sites) sets X velocity from a Byte index; `[B5]` (0 sites) sets Y velocity; `[B6]` (0 sites) sets both. Same table as [Animation Setup](families/anim_setup.md) but changes velocity independently without resetting animation state. Total: 10 call sites.
...
## Zero-use slots in range

| Op | Name | Notes |
|----|------|-------|
| `14` | `branch_if_rel_x_facing` | uses=0; params=Byte, Byte, &Code |
| `15` | `branch_if_rel_y_facing` | uses=0; params=Byte, Byte, &Code |
| `16` | `branch_if_actor_at_xy` | uses=0; params=Byte×4, &Code |
| `3F` | `apu_write_0` | uses=0; params=Byte |
| `40` | `apu_write_1` | uses=0; params=Byte |
| `43` | `queue_sfx_word` | uses=0; params=Word |
| `4F` | `refresh_tile_at` | uses=0; params=Byte, Byte |
| `60` | `award_exp` | uses=0; params=Word; handler fully traced |
| `70` | `npc_busy_wait` | uses=0; params=Byte; simplified `[6F]` variant |
| `7F` | `unused_crash` | uses=0; jump table entry `#$FFFF` → crashes if reached |
| `96` | `set_bitmap_overlay_spd` | uses=0; speed variant of `[95]` |
| `A1` | `bitmap_render_wait_multi` | uses=0; multi-frame bitmap wait |
| `A4` | `spawn_actor_child_flags` | uses=0; child spawn + flags word |
| `A6` | `spawn_actor_child_offset_flags` | uses=0; child spawn + facing offset + flags |
| `A7` | `spawn_actor_child_xy` | uses=0; child spawn + absolute X/Y |
| `A8` | `spawn_actor_child_xy_flags` | uses=0; child spawn + absolute X/Y + flags |
| `B5` | `set_velocity_y` | uses=0; Y velocity via table lookup |
| `B6` | `set_velocity_xy` | uses=0; both X+Y velocity via table lookup |
| `DE` | `toggle_vflip` | uses=0; toggles v-flip in `$0A` |
| `E2` | `proximity_branch_toward` | uses=0; proximity-based 3-way branch |
| `E9` | `branch_party_near_behind` | uses=0; reverse-facing proximity check |
